...
from src.slstm_attn_fMRI import LSTMTrainer
...
def train_encoder(args):
    ...
    Name = args.exp + '_FBIRN_' + args.pre_training + 'fMRI_2d1TimePoint'
    dir = 'run-' + d1 + d2 + Name
    dir = dir + '-' + str(ID)
    wdb = 'wandb_new'
    ...
    if args.exp == 'FPT':
        ...
        gain = [0.15, 0.25, 0.5, 0.9, 0.5]  # NPT
    ...
    # For ICA TC Data
    fix_z=22
    data = np.zeros((subjects, sample_x, sample_y, tc))
...
#            data[p, :, :, :] = img_np[:, :, fix_z, :tc]

#    with open('../fMRI/FBIRN/fMRI_Slice_FBIRN.npz', 'wb') as filesim:
#        np.save(filesim, data)

    with open('../fMRI/FBIRN/fMRI_Slice_FBIRN.npz', 'rb') as file:
        data = np.load(file)
    ...
    if args.fMRI_twoD:
        ...
        finalData2 = finalData2.permute(0, 3, 1, 2)
        finalData2 = finalData2.reshape(finalData2.shape[0], finalData2.shape[1], finalData2.shape[2], finalData2.shape[3], 1)
    else:
        finalData2 = np.zeros((subjects, samples_per_subject, sample_x, sample_y, window_size))
        for i in range(subjects):
            for j in range(samples_per_subject):
                finalData2[i, j, :, :, :] = data[i, :, :, j * window_shift:(j * window_shift) + window_size]
        finalData2 = torch.from_numpy(finalData2).float()
    ...
    if (args.fMRI_twoD):
        ...
        finalData2 = finalData2.reshape(finalData2.shape[0], finalData2.shape[1], finalData2.shape[2], finalData2.shape[3] )
        finalData2 = finalData2.reshape(finalData2.shape[0], finalData2.shape[1], 1, finalData2.shape[2], finalData2.shape[3] )
    else:
        finalData2 = finalData2.reshape(finalData2.shape[0], finalData2.shape[1], 1, finalData2.shape[2],
                                        finalData2.shape[3], finalData2.shape[4])
            ...
            # for g_trial in range (ngtrials):
                ...
                if args.encoder_type == "Nature":
                    ...
                    if args.pre_training == "milc-fMRI":
                        dir = 'PreTrainedEncoders/Milc_fMRI_slice_2dconv_1timepoint_32Chnls_deep_1024_512_multi_head_attention/encoder.pt'
                        args.oldpath = wpath1 + '/PreTrainedEncoders/Milc_fMRI_slice_2dconv_1timepoint_32Chnls_deep_1024_512_multi_head_attention'
                ...
                if args.method == 'cpc':
                    ...
                    trainer = LSTMTrainer(encoder, lstm_model, config, device=device, device_encoder=device_one, tr_labels=tr_labels,
                ...
                results[xindex][0], results[xindex][1], results[xindex][2], results[xindex][3] = trainer.train(tr_eps, val_eps, test_eps)
