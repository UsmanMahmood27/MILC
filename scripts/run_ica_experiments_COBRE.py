...
def train_encoder(args):
    ...
    d2 = str(JobID) + '_' + str(ID) + '_' + d1
    Name = args.exp + '_COBRE_RSEAM'
    dir = 'run-' + d1 + d2 + Name
    dir = dir + '-' + str(ID)
    wdb = 'wandb_new'
    ...
    #     filename = '../TC_label_COBRE/COBRE_ica_br' + str(p + 1) + '.csv'
    #     if p % 30 == 0:
    #         print(filename)
    ...
    total_HC_index_tr = HC_index[:len(HC_index) - (14 + 14)]
    total_SZ_index_tr = SZ_index[:len(SZ_index) - (16 + 16)]

    # HC_index_val = HC_index[len(HC_index) - (32 + 16):len(HC_index) - 32]
    # SZ_index_val = SZ_index[len(HC_index) - (32 + 16):len(HC_index) - 32]

    HC_index_val = HC_index[len(HC_index) - (14 + 14):len(HC_index) - 14]
    SZ_index_val = SZ_index[len(HC_index) - (16 + 16):len(HC_index) - 16]

    HC_index_test = HC_index[len(HC_index) - (14):]
    SZ_index_test = SZ_index[len(SZ_index) - (16):]
    ...
    for trial in range(ntrials):
        ...
        # encoder = NatureCNN(input_channels, args)
            ...
            if args.pre_training == "basic":
                ...
                dir = 'PreTrainedEncoders/MilcWithTwoLosses/encoder.pt'
                args.oldpath = wpath1 + '/PreTrainedEncoders/MilcWithTwoLosses'
    ...
    # hfn = h5py.File('../COBRE_HC_TrainingIndex.h5', 'w')
    # hfn.create_dataset('HC_TrainingIndex', data=torch.stack(HCrand))
    # hfn.close()
    #
    # hfq = h5py.File('../COBRE_SZ_TrainingIndex.h5', 'w')
