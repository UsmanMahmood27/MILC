...
from src.encoders_LS_BWindow import NatureCNN, ImpalaCNN, NatureOneCNN
...
def train_encoder(args):
    ...
    # For ICA TC Data
    data = np.zeros((193, 128, 20000))
    # modifieddata = np.zeros((823, 100, 1100))
    finalData = np.zeros((193, 285, 128, 70))
    finalData2 = np.zeros((193, 285, 128, 70))
    ...
    # print(data.shape)
    hf = h5py.File('../libreSpeech_AllData_New.h5', 'r')
    data2 = hf.get('libreSpeech_dataset_New')
    data2 = np.array(data2)
    data2 = data2.reshape((193, 128, 20000))
    data = data2

    for i in range(193):
        for j in range(285):
            finalData[i, j, :, :] = data[i, :, (j * 70):(j * 70) + 70]

    print(finalData.shape)
    filename = 'index_array_labelled_LS_New.csv'
    df = pd.read_csv(filename, header=None)
    index_array = df.values
    index_array = torch.from_numpy(index_array).int()
    index_array = index_array.view(193)
    ...
    tr_index2 = index_array[test_indexe:193]
