...
def train_encoder(args):
    ...
    #    # print(p)
    #    filename = '../fMRI/ABIDE/Right/SM' + str(p + 1) + '.nii'
    ...
    #
    #with open('../fMRI/ABIDE/fMRI_full_ABIDE.npz', 'wb') as filesim:
    ...
    with open('../fMRI/ABIDE/fMRI_full_ABIDE.npz', 'rb') as file:
    ...
    for trial in range(ntrials):
        ...
        # Choose the subject_per_class indices from HC_index_val and SZ_index_val using random numbers
            ...
            if args.encoder_type == "Nature":
                encoder = NatureCNN(observation_shape[2], args, device=device, device_one=device_encoder,
                                    device_two=device_encoder, device_three=device)
            ...
            complete_model = combinedModel(encoder, lstm_model, gain=current_gain, PT=args.pre_training, exp=args.exp,
            ...
            results[trial][0], results[trial][1], results[trial][2], results[trial][3] = trainer.pre_train(tr_eps, val_eps,
                                                                                                       test_eps)
