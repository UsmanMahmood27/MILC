...
def train_encoder(args):
    ...
    Name = args.exp + '_OASIS_'+args.pre_training
    dir = 'run-' + d1 + d2 + Name
    dir = dir + '-' + str(ID)
    wdb = 'wandb_new'
    ...
    # os.mkdir(fig_path)
    # hf = h5py.File('../OASIS3_AllData.h5', 'w')
    ntrials = 10
    ngtrials = 10
    best_auc=0.
    best_gain=0
    # current_gain=0
    tr_sub = [15, 25, 50, 75, 100, 120]

    # gain = [0.45, 0.65, 0.8, 0.3, 0.25, 0.8]     #FPT
    # gain = [0.15, 0.75, 0.45, 0.85, 0.25, 0.55]    #UFPT
    # gain = [0.25, 0.35, 0.75, 0.30, 0.4, 0.35]     #NPT
    # gain

    # For encoder trained on OASIS
    # gain = [0.1, 0.5, 0.35, 0.3, 0.7, 0.4]     #FPT
    # gain = [0.45, 0.15, 0.3, 0.35, 0.65, 0.65]    #UFPT
    # gain = [0.25, 0.35, 0.75, 0.30, 0.4, 0.35]     #NPT
    ...
    # variable attention default
    if args.exp == 'FPT':
        gain = [0.1, 0.5, 0.35, 0.3, 0.7, 0.4]  # FPT
    elif args.exp == 'UFPT':
        gain = [0.05, 0.15, 0.3, 0.35, 0.65, 0.35]  # UFPT
    else:
        gain = [0.25, 0.35, 0.75, 0.30, 0.4, 0.35]  # NPT
    ...
    samples_per_subject = 11
    # samples_per_subject = int((tc - sample_y)+1)
    ntest_samples_perclass = 32
    nval_samples_perclass = 32
    test_indices = [0, 32, 64, 96, 128, 154]
    ...
    #     filename = '../TC_label_OASIS3/OASIS3_ica_br' + str(p + 1) + '.csv'
    #     if p % 30 == 0:
    #         print(filename)
    ...
    results = torch.zeros(ntrials * 6, 4)
    ...
    for test_ID in range(6):
        ...
        for trial in range(ntrials):
            ...
            # Get subject_per_class number of random values
                ...
                # SZrand.append(torch.randperm(total_HC_index_tr.shape[0]))
                # print(torch.randperm(122))
                ...
                results[xindex][0], results[xindex][1], results[xindex][2], results[xindex][3] = trainer.train(tr_eps, val_eps,
                                                                                                            test_eps)
    ...
    # print('a')
    # hfn = h5py.File('../OASIS3_HC_TrainingIndex.h5', 'w')
    # hfn.create_dataset('HC_TrainingIndex', data=torch.stack(HCrand))
    # hfn.close()
    #
    # hfq = h5py.File('../OASIS3_SZ_TrainingIndex.h5', 'w')
