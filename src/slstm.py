...
class LSTMTrainer(Trainer):
    def __init__(self, encoder, lstm, config, device, tr_labels, val_labels, test_labels, wandb=None, trial="", crossv="", gtrial=""):
        ...
        if self.exp in ['UFPT', 'NPT']:
            self.optimizer = torch.optim.Adam(list(self.lstm.parameters()) + list(self.encoder.parameters()),
    ...
    def do_one_epoch(self, epoch, episodes, mode):
        ...
        for sx, ts_number in data_generator:
            ...
            # print('input ', inputs[0].device.type)
            outputs = self.lstm(inputs, mode)
            # print('output ', outputs[0].device.type)
            logits = torch.stack(outputs)
            # print('logits ', logits.device.type)
