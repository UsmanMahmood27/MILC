...
from .utils_attention_realData import calculate_accuracy, Cutout, calculate_accuracy_by_labels, calculate_false_positive
...
class LSTMTrainer(Trainer):
    def __init__(self, encoder, lstm, config, device, exp, tr_labels, val_labels, test_labels, wandb=None):
        ...
        if self.exp in ['unfrozen', 'full']:
            self.optimizer = torch.optim.Adam(list(self.decoder.parameters())+list(self.attn.parameters())+list(self.lstm.parameters())+list(self.encoder.parameters()), lr=config['lr'], eps=1e-5)
        else:
            self.optimizer = torch.optim.Adam(list(self.decoder.parameters())+list(self.attn.parameters())+list(self.lstm.parameters()),lr=config['lr'], eps=1e-5)
        self.early_stopper = EarlyStopping(patience=self.patience, epochs= self.epochs, verbose=False, wandb=self.wandb, encname=self.encoder, lstmname=self.lstm, attnname=self.attn, decodename=self.decoder, path=self.path)
    ...
    def generate_batch(self, episodes, mode):
        total_steps = len(episodes)  #How many samples will be generated in total
        ...
        for indices in sampler:
            ...
            yield torch.stack(subjects).to(self.device), ts_number.to(self.device)
    ...
    def compute_saliency_maps(self, X, y, subjects_per_group, trial_no):
        ...
        for X in outputs:
            result = [self.attn(torch.cat((X[i], X[-1]), 0)) for i in range(121)]
    ...
    def do_one_epoch(self, epoch, episodes, mode, subjects_per_group, trial_no):
        ...
        epoch_loss1, epoch_loss2, epoch_accuracy, epoch_roc, steps = 0., 0., 0., 0., 0
        accuracy1, accuracy2, accuracy = 0., 0., 0.
        roc = 0.0
        ...
        for subjects, indices_m in data_generator:
           ...
           # print('After attention shape:', attn_applied.shape)
            ...
            accuracy = calculate_accuracy_by_labels(indices, labels.to(self.device))
        ...
        self.log_results(subjects_per_group, trial_no, epoch, epoch_loss1 / steps, epoch_loss2 / steps, epoch_accuracy / steps, epoch_roc / steps, prefix=mode)

        if mode == "val":
            self.early_stopper(epoch, -epoch_loss2 / steps, epoch_accuracy / steps, self.encoder, self.lstm, self.attn, self.decoder)
            if epoch % 10 == 0:
                print("Best Epoch So Far: {}, Best Loss So Far: {}, Best Accuracy So Far: {}".format(self.early_stopper.best_epoch,
                -self.early_stopper.val_loss_max, self.early_stopper.best_acc))
    ...
    def save_predictions(self, indices, labels, subjects_per_group, trial_no):
        ...
        filename = os.path.join(path, self.exp + '_subj_' + str(subjects_per_group) + '_trial_' + str(trial_no) + '.csv')
        result.to_csv(filename)

    def do_test(self, epoch, episodes, mode, subjects_per_group, trial_no):

        epoch_loss, epoch_accuracy, epoch_roc, steps = 0., 0., 0., 0
        accuracy1, accuracy2, accuracy = 0., 0., 0.
        data_generator = self.generate_batch(episodes, mode)
        start_time = time.time()
        for subjects, indices_m in data_generator:
           ...
           # print('Weights Shape:', weights.shape)
            ...
            values, indices = sig.max(1)
            accuracy = calculate_accuracy_by_labels(indices, labels.to(self.device))
            self.save_predictions(indices, labels, subjects_per_group, trial_no)
            targets = labels
            y_scores = sig.to(self.device).detach()[:, 1]
            roc = roc_auc_score(targets.to('cpu'), y_scores.to('cpu'))
            saliency = self.compute_saliency_maps(subjects, labels, subjects_per_group, trial_no)
            print('Saliency Shape:', saliency.shape)
    ...
    def train(self, tr_eps, val_eps, tst_eps, subjects_per_group, trial_no):
        ...
        scheduler = torch.optim.lr_scheduler.MultiStepLR(self.optimizer, milestones=[50, 120], gamma=0.35)
        for e in range(self.epochs):
            mode="train"
            training_accuracy[e], training_loss1[e], training_loss2[e] =\
                self.do_one_epoch(e, tr_eps, mode, subjects_per_group, trial_no)
            mode="val"
            with torch.no_grad():
                val_accuracy[e], val_loss1[e], val_loss2[e]= \
                    self.do_one_epoch(e, val_eps, mode, subjects_per_group, trial_no)
            scheduler.step()
            if self.early_stopper.early_stop:
                req_epoch = e
                break

        self.save_loss_accuracy(training_accuracy, training_loss1, training_loss2, val_accuracy, val_loss1, val_loss2, subjects_per_group, trial_no)
        self.load_model()
        print('Now One batch test (do test) is running.....')
        mode = "test"
        test_accuracy, test_loss, test_auc = self.do_test(e, tst_eps, mode, subjects_per_group, trial_no)
        print("Test Accuracy: {}, Test Loss: {}, Test Area Under Curve: {}".format(test_accuracy, test_loss, test_auc))
       #self.plot_loss_accuracy(training_accuracy, val_accuracy, req_epoch)
        return test_accuracy, test_loss, test_auc, req_epoch

    def log_results(self, subjects_per_group, trial_no, epoch_idx, epoch_loss1, epoch_loss2, epoch_accuracy, roc, prefix=""):
        print("{} Subj Per Group: {}, Trial: {}, Epoch: {}, Epoch Loss1: {}, Epoch Loss2: {}, Epoch Accuracy: {}, AUC: {}, {}".format(prefix.capitalize(), subjects_per_group, trial_no, epoch_idx, epoch_loss1, epoch_loss2, epoch_accuracy, roc, prefix.capitalize()))


    def save_loss_accuracy(self, training_accuracy, training_loss1, training_loss2, val_accuracy, val_loss1, val_loss2, subjects_per_group, trial_no):
        ...
        filename = os.path.join(path, self.exp + '_CE_'+'_subj_' + str(subjects_per_group) + '_trial_' + str(trial_no) + '.csv')
        ...
        filename = os.path.join(path, self.exp + '_CEPlusL1_' + '_subj_' + str(subjects_per_group) + '_trial_' + str(trial_no) + '.csv')
    ...
    def load_model(self):
        encoder_dict = torch.load(os.path.join(self.path, 'encoder.pt'), map_location=self.device)
        self.encoder.load_state_dict(encoder_dict)
        self.encoder.eval()
        self.encoder.to(self.device)

        lstm_dict = torch.load(os.path.join(self.path, 'lstm.pt'), map_location=self.device)
        self.lstm.load_state_dict(lstm_dict)
        self.lstm.eval()
        self.lstm.to(self.device)

        attn_dict = torch.load(os.path.join(self.path, 'attn.pt'), map_location=self.device)
        self.attn.load_state_dict(attn_dict)
        self.attn.eval()
        self.attn.to(self.device)

        decoder_dict = torch.load(os.path.join(self.path, 'decoder.pt'), map_location=self.device)
    ...
    def plot_loss_accuracy(self, training_accuracy, val_accuracy, req_epoch):
        f = plt.figure()
        plt.plot(self.batch_train_loss, 'g', label='Training Loss', linewidth=2)
        plt.plot(self.batch_val_loss, 'r', label='Validation Loss', linewidth=2)
        plt.xlabel('Batches')
        plt.ylabel('Loss')
        ...
        f = plt.figure()
        plt.plot(range(req_epoch), training_accuracy[:req_epoch], 'g', label='Training Accuracy', linewidth=2)
        plt.plot(range(req_epoch), val_accuracy[:req_epoch], 'r', label='Val Accuracy', linewidth=2)
        plt.xlabel('Epochs')
        plt.ylabel('Accuracy')
        plt.title('Learning Curves')
        plt.legend()
        plt.show()
        f.savefig(os.path.join(self.fig_path, 'accuracy.png'), bbox_inches='tight')

    def eval_on_train(self, tr_eps, val_eps, test_eps, subjects_per_group, trial_no):
        self.load_model()
        trial_no = 100
        print('Now One batch test (do test) is running.....')
        mode = "test"
        ...
        test_accuracy, test_loss, test_auc = self.do_test(e, test_eps, mode, subjects_per_group, trial_no)
        print("Test Accuracy: {}, Test Loss: {}, Test Area Under Curve: {}".format(test_accuracy, test_loss, test_auc))
