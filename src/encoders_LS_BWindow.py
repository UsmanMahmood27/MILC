...
class NatureCNN(nn.Module):
    def __init__(self, input_channels, args):
        ...
        if self.downsample:
            ...
            self.final_conv_size = 64 * 2 * 12
            self.final_conv_shape = (64, 2, 12)
            self.main = nn.Sequential(
                init_(nn.Conv2d(input_channels, 32, 3, stride=1)),
                nn.ReLU(),
...
class NatureOneCNN(nn.Module):
    def __init__(self, input_channels, args):
        ...
        if self.downsample:
            self.final_conv_size = 256 * 644
            self.final_conv_shape = (256, 644)
            self.main = nn.Sequential(
                init_(nn.Conv1d(input_channels, 128, 100, stride=1)),
                nn.ReLU(),
                init_(nn.Conv1d(128, 200, 100, stride=1)),
                nn.ReLU(),
                init_(nn.Conv1d(200, 256, 80, stride=1)),
                nn.ReLU(),
                init_(nn.Conv1d(256, 256, 80, stride=1)),
                nn.ReLU(),
                Flatten(),
                init_(nn.Linear(self.final_conv_size, self.feature_size)),
                init_(nn.Conv1d(256, 128, 80, stride=1)),
                nn.ReLU(),
                #nn.ReLU()
            )
        else:
            self.final_conv_size = 256 * 38
            self.final_conv_shape = (256, 38)
            self.main = nn.Sequential(
                init_(nn.Conv1d(input_channels, 128, 10, stride=1)),
                nn.ReLU(),
                init_(nn.Conv1d(128, 200, 10, stride=1)),
                nn.ReLU(),
                init_(nn.Conv1d(200, 256, 8, stride=1)),
                nn.ReLU(),
                init_(nn.Conv1d(256, 256, 8, stride=1)),
                nn.ReLU(),
                Flatten(),
                init_(nn.Linear(self.final_conv_size, self.feature_size)),
                init_(nn.Conv1d(256, 128, 8, stride=1)),
                nn.ReLU(),
                # nn.ReLU()
            )
        self.train()
    ...
    def forward(self, inputs, fmaps=False, five=False):
        f5 = self.main[:8](inputs)
        # f7 = self.main[6:8](f5)
        out = self.main[8:10](f5)
        f5 = self.main[10:](f5)
