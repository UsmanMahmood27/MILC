...
class NatureOneCNN(nn.Module):
    def __init__(self, input_channels, args):
        ...
        if self.downsample:
        ...
        elif self.small_window:
            self.final_conv_size = 64 * 6
            self.final_conv_shape = (64, 6)
            self.main = nn.Sequential(
                init_(nn.Conv1d(input_channels, 32, 2, stride=1)),
                nn.ReLU(),
                init_(nn.Conv1d(32, 64, 2, stride=1)),
                nn.ReLU(),
                init_(nn.Conv1d(64, 128, 2, stride=1)),
                nn.ReLU(),
                init_(nn.Conv1d(128, 64, 2, stride=1)),
                nn.ReLU(),
                Flatten(),
                init_(nn.Linear(self.final_conv_size, self.feature_size)),
            )
            ...
            self.final_conv_size = 64 * 11
            self.final_conv_shape = (64, 11)
            self.main = nn.Sequential(
                init_(nn.Conv1d(input_channels, 32, 4, stride=1)),
                nn.ReLU(),
                init_(nn.Conv1d(32, 64, 4, stride=1)),
                nn.ReLU(),
                init_(nn.Conv1d(64, 128, 3, stride=1)),
                nn.ReLU(),
                init_(nn.Conv1d(128, 64, 2, stride=1)),
                nn.ReLU(),
                Flatten(),
...
class NatureOneNewCNN(nn.Module):
    def __init__(self, input_channels, args):
        super().__init__()
        self.feature_size = args.feature_size
        self.hidden_size = self.feature_size
