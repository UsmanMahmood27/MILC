...
class LSTMTrainer(Trainer):
    def __init__(self, encoder, lstm, config, device, tr_labels, val_labels, test_labels, wandb=None, trial="", gtrial=""):
    ...
    def do_one_epoch(self, epoch, episodes, mode):
        ...
        for sx, ts_number in data_generator:
            ...
            logits = torch.stack(outputs).to(self.device)
            ...
            #if mode == "eval":
                #self.eval_batch_loss.append(loss.detach().item())
            #elif mode == 'train':
                #self.train_batch_loss.append(loss.detach().item())
        ...
        if mode == "eval":
            self.early_stopper(epoch_loss / steps, epoch_roc / steps, self.encoder, self.lstm, 0)
        if mode == 'test':
    ...
    def train(self, tr_eps, val_eps, tst_eps):
        ...
        for e in range(self.epochs):
            ...
            if self.early_stopper.early_stop:
                self.early_stopper(0, 0, self.encoder, self.lstm, 1)
                saved = 1
                break
        ...
        # f = pl.figure()
        #
        # pl.semilogy(self.train_epoch_loss[1:], label='train_total_loss')
        # pl.semilogy(self.eval_epoch_loss[1:], label='val_total_loss')
        # pl.semilogy(self.eval_epoch_CE_loss[1:], label='val_CE_loss')
        # pl.semilogy(self.eval_epoch_E_loss[1:], label='val_Enc_loss')
        # pl.semilogy(self.eval_epoch_lstm_loss[1:], label='val_lstm_loss')
        # #
        #
        # pl.xlabel('epochs')
        # pl.ylabel('loss')
    ...
    def log_results(self, epoch_idx, epoch_loss1, epoch_loss, epoch_test_accuracy, epoch_FP, epoch_roc, prefix=""):
        print(
            "{} Trial: {}, Gain_Trial: {}, Epoch: {}, Epoch Loss: {}, Epoch Accuracy: {}, Epoch FP: {} roc: {},  {}".format(
                prefix.capitalize(),
                self.trials,
                self.gtrial,
                epoch_idx,
