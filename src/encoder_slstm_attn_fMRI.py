...
class LSTMTrainer(Trainer):
    def __init__(self, encoder, lstm, config, device, device_encoder, wandb=None, trial="", crossv="", gtrial=""):
        ...
        self.classifier1 = nn.Sequential(
            nn.Linear(config['fMRI_feature_size'], config['fMRI_lstm_size']),

        ).to(device)

        self.classifier4 = nn.Sequential(
            nn.Linear(config['lstm_size'], config['fMRI_lstm_size']),
        ...
        self.classifier2 = nn.Sequential(
            nn.Linear(config['fMRI_lstm_size'], config['fMRI_lstm_size']),
            nn.ReLU(),
            nn.Linear(config['fMRI_lstm_size'], config['fMRI_lstm_size'])
        ...
        self.attn = nn.Sequential(
            nn.Linear(2 * config['fMRI_lstm_size'], 128),
            nn.ReLU(),
            nn.Linear(128, 1)
        ).to(device)
        ...
        self.multihead_attn = nn.MultiheadAttention(self.lstm.hidden_dim, self.n_heads).to(self.device)
        ...
        self.optimizer = torch.optim.Adam(list(self.attn.parameters()) + list(self.lstm.parameters()) + list(
            ...
            self.classifier1.parameters()) + list(self.key_layer.parameters())
                                                  + list(self.value_layer.parameters()) +
                list(self.query_layer.parameters()) + list(self.multihead_attn.parameters()), lr=config['lr'], eps=1e-5)
    ...
    def do_one_epoch(self, epoch, episodes, mode):
        #print('lstm device one epoch start ' + torch.cuda.get_device_name(device=self.device)  + ' ', (torch.cuda.memory_allocated(device=self.device) / (1024*1024)))
        #print('encoder device one epoch start  ' + torch.cuda.get_device_name(device=self.device_encoder)  + ' ', (torch.cuda.memory_allocated(device=self.device_encoder) / (1024*1024)))
        ...
        for sx, ts_number in data_generator:
            ...
            #print('lstm device for start  ' + torch.cuda.get_device_name(device=self.device) + ' ', (torch.cuda.memory_allocated(device=self.device) / (1024*1024)))
            #print('encoder device for start  '  + torch.cuda.get_device_name(device=self.device_encoder) + ' ', (torch.cuda.memory_allocated(device=self.device_encoder) / (1024*1024)))
            # print("in sx")
            ...
            inputs_tensor = torch.stack(inputs).to(self.device)
            outputs = self.lstm(inputs_tensor, mode)
            ...
            logits = self.multi_head_attention(outputs)
            ...
            for y in range(sy):
                ...
                # lgt = logits[y_index, v, :].clone().squeeze()
                ...
                sig = torch.softmax(mlogits.detach(), dim=1).to(self.device)
        ...
        #print('lstm device for end  ' + torch.cuda.get_device_name(device=self.device) + ' ', (torch.cuda.memory_allocated(device=self.device) / (1024*1024)))
        #print('encoder device for end  ' + torch.cuda.get_device_name(device=self.device_encoder) + ' ', (torch.cuda.memory_allocated(device=self.device_encoder) / (1024*1024)))
        if mode == "eval":
        ...
        if mode == "eval":
            self.early_stopper(epoch_loss / steps, epoch_accuracy / steps, self.encoder, self.lstm, self.attn,
                               self.classifier1, 0, self.key_layer, self.value_layer, self.query_layer, self.multihead_attn)
        if mode == 'test':
            self.test_accuracy = epoch_accuracy / steps
            self.test_auc = epoch_roc / steps
            self.test_loss = epoch_loss / steps

        #print('lstm device one epoch end  ' + torch.cuda.get_device_name(device=self.device) + ' ', (torch.cuda.memory_allocated(device=self.device) / (1024*1024)))
        #print('encoder device one epoch end  ' + torch.cuda.get_device_name(device=self.device_encoder) + ' ', (torch.cuda.memory_allocated(device=self.device_encoder) / (1024*1024)))
    ...
    def get_attention(self, outputs):
        ...
        #     wt = normalized_weights.clone().unsqueeze(1)
        #     wt[:, :, i] = 0
        #     attn_applied = torch.bmm(wt, outputs)
        #     attn_applied = attn_applied.squeeze()
        #     attention.append(attn_applied)
    ...
    def train(self, tr_eps, tst_eps):
        ...
        for e in range(self.epochs):
            self.encoder.train(), self.lstm.train(), self.multihead_attn.train()

            mode = "train"
            val_loss = self.do_one_epoch(e, tr_eps, mode)

            self.encoder.eval(), self.lstm.eval(), self.multihead_attn.eval()
            mode = "eval"
