...
class NatureOneCNN(nn.Module):
    def __init__(self, input_channels, args):
        ...
        if self.downsample:
            ...
            self.final_conv_size = 256 * 3
            self.final_conv_shape = (256, 3)
            self.main = nn.Sequential(
                init_(nn.Conv1d(input_channels, 128, 2, stride=1)),
                nn.ReLU(),
                init_(nn.Conv1d(128, 200, 2, stride=1)),
                nn.ReLU(),
                init_(nn.Conv1d(200, 256, 1, stride=1)),
                nn.ReLU(),
                Flatten(),
                init_(nn.Linear(self.final_conv_size, self.feature_size)),
                init_(nn.Conv1d(256, 128, 1, stride=1)),
                nn.ReLU(),
                # nn.ReLU()
            )
        self.train()
