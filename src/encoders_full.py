    ...
    def __init__(self, input_channels, args, device, device_one, device_two, device_three):
        ...
        if self.two_d:
            ...
            self.main = nn.Sequential(
                ...
                nn.ReLU(),
                init_(nn.Conv2d(16, 32, (9, 10), stride=1)).to(self.device_two),
                nn.ReLU(),
                init_(nn.Conv2d(32, 32, (8, 9), stride=1)).to(self.device_three),
                nn.ReLU(),
                init_(nn.Conv2d(32, 32, (7, 8), stride=1)).to(self.device),
                nn.ReLU(),
                Flatten(),
            ...
            self.final_conv_size = 16 * 24 * 30 * 6
            self.final_conv_shape = (16, 24, 30, 6)
            self.final_conv_size = 8 * 24 * 30 * 23
            self.final_conv_shape = (8, 24, 30, 23)
            self.main = nn.Sequential(
                init_(nn.Conv3d(self.input_channels, 8, (9, 10, 9), stride=(1, 1, 1))).to(self.device_one),
                nn.ReLU(),
                init_(nn.Conv3d(8, 8, (9, 10, 9), stride=(1, 1, 1))).to(self.device_two),
                nn.ReLU(),
                init_(nn.Conv3d(8, 8, (8, 9, 8), stride=(1, 1, 1))).to(self.device_three),
                nn.ReLU(),
                init_(nn.Conv3d(8, 8, (7, 8, 7), stride=(1, 1, 1))).to(self.device),
                nn.ReLU(),
                Flatten(),
                init_(nn.Linear(self.final_conv_size, self.feature_size).to(self.device)),
