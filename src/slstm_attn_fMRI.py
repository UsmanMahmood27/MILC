...
class LSTMTrainer(Trainer):
    def __init__(self, encoder, lstm, config, device, device_encoder, tr_labels, val_labels, test_labels, wandb=None, trial="", crossv="", gtrial=""):
        ...
        if self.exp in ['UFPT', 'NPT']:
            self.optimizer = torch.optim.Adam(
                list(self.decoder.parameters()) + list(self.lstm.parameters()) + list(
                    self.encoder.parameters()) + list(self.key_layer.parameters())
                                                  + list(self.value_layer.parameters()) +
                list(self.query_layer.parameters()) + list(self.multihead_attn.parameters())
                + list(self.final_attn.parameters()) ,lr=config['lr'], eps=1e-5)
        else:
    ...
    def add_regularization(self, loss):
        ...
        loss = loss + E_loss + lstm_loss + attn_loss + mha_loss
